...
class FileSize extends Command {
    ...
    async run(message, args, level) {
        const file = args[0];
        if (!file) {
            return message.channel.send("You must provide a file (and location if non-root file), and the correct syntax must be used.");
        }
        try {
            ...
            if (err.code === "ENOENT") {
                return message.channel.send(`The file \`${file}\` could not be found.`);
            } else {
