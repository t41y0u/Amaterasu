...
class BigLetter extends Command {
    ...
    async run(message, args, level) {
        const generate = () => {
            let text = args.join(" ");
            if (!text) return this.client.embed("commonError", message, "Please provide some text for me to convert to regional indicators"); 
            ...
            if (!isAlphaNumeric(text)) {
                return this.client.embed("commonError", message, "Invalid character(s) provided.\nPlease ensure you have only provided alphanumeric characters.");
            }
            let output = "";
