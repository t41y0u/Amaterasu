...
function play(guild, song) {
    ...
    if (!song) {
        ...
        guild.client.user.setActivity(`with Sakuta-kun • m|help`);
        return;
    }
    const dispatcher = queue.get(guild.id).connection.playStream(ytdl(song.url, {quality:"highest", filter:"audioonly"}, {passes: 3, volume: guild.voiceConnection.volume || 1}))
    .on("end", () => {
        if (!serverQueue.loop) {
    ...
    guild.client.user.setActivity(`🎶 | ${song.title}`);
    const embed = new RichEmbed()
